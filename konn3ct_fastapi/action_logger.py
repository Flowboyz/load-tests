...
class ActionLogger:
    ...
    async def _periodic_flush(self):
        while True:
            try:
                ...
                print(f"Error in ActionLogger periodic flush: {e}", file=sys.stderr)
    ...
    async def flush(self):
        ...
        # Write to disk in a separate thread to prevent event loop blocking
        def write_disk():
            with open(self.log_path, "a", encoding="utf-8") as f:
                f.write(lines)
        
        try:
            await asyncio.to_thread(write_disk)
        except Exception as e:
            print(f"Error writing action logs to disk: {e}", file=sys.stderr)
    ...
    async def record_event(self, event_type, bot_id=None, name=None, email=None, **extra):
        ...
        async with self.lock:
            ...
            # If buffer size is large, trigger an immediate flush in background
