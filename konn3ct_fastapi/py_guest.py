...
async def log_observed_action(bot_id, name, email, uid, action_type, value, fingerprint, msg_client_event_id=None, msg_server_event_id=None):
    ...
    """
    # Telemetry sampling rate: only log observations for a subset of bots when total_bots > 50
    if TOTAL_BOTS > 50:
        sample_rate = 20.0 / TOTAL_BOTS
...
async def action_loop(
    ...
    while not stop_event.is_set():
        ...
        if now >= next_action_at:
            ...
            if role == "host":
                for host_act in ["force_mute", "remove_participant", "lock_meeting", "recording_state"]:
                    if host_act in scenarios:
                        choices.append(host_act)
            choices.append("captions_state")
            
            # Viewer Stability & Host/Presenter Early-Exit Protection
            is_viewer = is_bot_in_range(bot_id, getattr(args, 'viewer_bots', '6-10000'))
...
# Main orchestrator
async def main(args):
    global logger, TOTAL_BOTS
    stop_event = asyncio.Event()
    TOTAL_BOTS = args.total_bots if (hasattr(args, 'total_bots') and args.total_bots and args.total_bots > 0) else args.bots
    
    if hasattr(args, 'mock_signaling') and args.mock_signaling:
        global active_mock_sockets
        active_mock_sockets = {}

        class MockWebSocket:
            def __init__(self, bot_id, room_id):
                self.bot_id = bot_id
                self.room_id = room_id
                self.user_id = f"user_{bot_id}"
                self.incoming_queue = asyncio.Queue()
                self.closed = False
                
            async def __aenter__(self):
                await self.incoming_queue.put(json.dumps({
                    "type": "session_status",
                    "status": "active",
                    "userId": self.user_id
                }))
                active_mock_sockets[self.user_id] = self
                return self
                
            async def __aexit__(self, exc_type, exc_val, exc_tb):
                self.closed = True
                active_mock_sockets.pop(self.user_id, None)
                
            async def send(self, data):
                req = json.loads(data)
                mtype = req.get("type")
                client_event_id = req.get("clientEventId") or req.get("clientMsgId") or f"cli_{self.bot_id}_{int(time.time()*1000)}"
                server_event_id = f"srv_{client_event_id}"
                ...
                for k, v in req.items():
                    if k not in ("type", "clientEventId"):
                        ack_msg[k] = v
                await self.incoming_queue.put(json.dumps(ack_msg))
                
                # Broadcast observation message to all other connected bots
                obs_msg = {
                    "type": mtype,
                    "userId": self.user_id,
                    "clientEventId": client_event_id,
                ...
                for uid, sock in list(active_mock_sockets.items()):
                    if uid != self.user_id:
                        try:
                            await sock.incoming_queue.put(json.dumps(obs_msg))
                        except Exception:
                            pass
        ...
        def mock_connect(uri, **kwargs):
            import base64
            bot_id = 1
            room_id = "strategy-meeting"
            try:
                if "token=" in uri:
                    token_start = uri.find("token=") + 6
                    token_end = uri.find("&", token_start)
                    token = uri[token_start:token_end] if token_end != -1 else uri[token_start:]
                    payload_b64 = token.split(".")[1]
                    payload_b64 += "=" * ((4 - len(payload_b64) % 4) % 4)
                    payload = json.loads(base64.b64decode(payload_b64).decode('utf-8'))
                    bot_id = int(payload.get("userId").split("_")[1])
            except Exception:
                pass
            try:
                if "roomId=" in uri:
                    room_start = uri.find("roomId=") + 7
                    room_end = uri.find("&", room_start)
                    room_id = uri[room_start:room_end] if room_end != -1 else uri[room_start:]
            except Exception:
                pass
            return MockWebSocket(bot_id, room_id)

        import websockets
        websockets.connect = mock_connect
        print("Mock signaling server activated for this process chunk.")
    ...
    await logger.record_event(
        ...
        browser_launch_options=args.browser_launch_options,
        sla_thresholds=args.sla_thresholds
    )
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        tasks = []
        bot_id = getattr(args, 'bot_start_id', 1)
        ...
        limit_id = getattr(args, 'bot_start_id', 1) + args.bots
        while bot_id < limit_id and not stop_event.is_set():
            await pause_event.wait()
            batch = []
            for i in range(args.batch):
                ...
                bot_id += 1
            tasks.extend(batch)
            if bot_id < limit_id and args.stagger > 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--browser-launch-options", default=None, help="JSON string representing Chromium launch options")
    parser.add_argument("--sla-thresholds", default=None, help="JSON string representing SLA thresholds")
    parser.add_argument("--viewer-bots", default="6-10000", help="Range of bot IDs that act as viewers/attendees in receive_only mode")
    parser.add_argument("--total-bots", type=int, default=0, help="Total bots in the entire test run")
    parser.add_argument("--mock-signaling", action="store_true", help="Run with in-memory mock signaling for local testing")
    parser.add_argument("--use-fake-ui-for-media-stream", action="store_true", help="Bypass Chromium media stream UI confirmation popups")
    parser.add_argument("--bot-start-id", type=int, default=1, help="The starting ID for bot index allocation")
