...
class EventRecorder:
    """
    Appends one JSON object per line to a .jsonl file as events happen.
    generate_report.py reads this file afterwards to build the Word report.
    """
...
async def run_bot(
    ...
    while not stop_event.is_set() and attempt <= max_retries:
        ...
        if recorder:
            await recorder.record("bot_reconnect_attempt", bot_id, name, email,
                                   attempt=attempt)
    ...
    if recorder:
        await recorder.record("bot_session_ended", bot_id, name, email,
...
async def main(args):
    ...
    # Set up the event recorder (JSON log used by generate_report.py)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--report-log",      default="report_log.jsonl", help="Path to write JSON event log for report generation (default: report_log.jsonl)")
